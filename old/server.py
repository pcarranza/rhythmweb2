...
class RBRequest(BaseHTTPServer.BaseHTTPRequestHandler, Loggable):
    ...
    def handle_method(self, request_method):
        path = self.check_path(self.path)
        self.info('%s Method: %s' % (request_method, path))
        ...
        if not hasattr(instance, the_method):
            self.error(code=500, message='Object %s does not have a method %s' % (instance, request_method))
