...
class Component(object):
    ...
    def load_html(self):
        ...
        log.debug("Loading html file %s for component %s" % (self._path, self.name()))
        ...
        log.debug('Loading html %s for component %s' % (html_file, self.name()))
    ...
    def pre_render_template(self):
        log.debug('PRE Rendering template %s' % self.name())
    
    def _render_template(self, node):
        self.pre_render_template()
        log.debug('Rendering template %s' % self.name())
    ...
    def create_template(self, html):
        log.debug('Creating template for component %s' % self.name())
...
class BasePage(Component):
    ...
    def do_get(self, environ, response):
        ...
        log.debug('Getting template %s' % self.name())
        try:
            html = template.render()
            response('200 OK', self.do_headers())
            log.debug('Template %s rendered ok, sending OK response' % self.name())
            return html
        except Exception as e:
            log.debug('Template %s rendered bad, sending ERROR response: %s' % (self.name(), e))
    ...
    def do_post(self, environ, params, response):
        ...
        self._environ = environ
        log.debug('Processing post in BasePage for component %s' % self.name())
        self.post()
        return self.do_get(environ, response)
        
    def post(self):
        log.warning('No actual posting... override post in ' + self.name())
...
class BasePanel(Component):
    ...
    def render(self):
        template = self.create_template(self.load_html())
        log.debug('Rendering panel %s' % self.name())
